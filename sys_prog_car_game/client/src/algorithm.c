...
void dfs(MAP_t* dgist, int visited[MAP_ROW][MAP_COL], int row, int col, int steps, int current_score, int path_length) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (new_row >= 0 && new_row < MAP_ROW && new_col >= 0 && new_col < MAP_COL && !visited[new_row][new_col]) {
            ...
            dfs(dgist, visited, new_row, new_col, steps - 1, current_score + dgist->map[new_row][new_col].item, path_length + 1);
...
int getMaxScore(MAP_t* dgist, int start_row, int start_col, int **ret_best_path) {
    ...
    printf("dgist->map[start_row][start_col].item : %d\n", dgist->map[start_row][start_col].item);
    dfs(dgist, visited, start_row, start_col, 4, dgist->map[start_row][start_col].item, 1);
